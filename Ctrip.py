...
class Ctrip(object):
	...
	def get_hotel(self, html):
		...
		htllist = re.findall('htllist: \'\[(.*?)\]\'', html, re.S)
		print(htllist)
		if htllist:
			amount = re.findall('amount":"(\d+)"', htllist[0], re.S)
			print(amount)
		else:
			print('htllist/price is False')
...
if __name__ == '__main__':
	...
	if check:
		...
		ch_in = time.strftime("%Y-%m-%d")
	ck_out = Ck_out()
	check2 = input('离店时间：xxxx-xx-xx')  # 2019-03-26
	if check2:
		ck_out = ck_out.check_out(check2)
	else:
		ck_out = (date.today() + timedelta(days=+1)).strftime("%Y-%m-%d")
